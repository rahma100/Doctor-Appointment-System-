  ...
  $object->connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	...
	if($_POST['action'] == 'fetch_appointment')
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '<a  href="download.php?id='.$row["appointment_id"].'" class="btn  btn-sm" target="_blank"><i class="fas fa-file-pdf"></i> PDF</a>';

			$sub_array[] = '<button style="background-color:#1977cc" type="button" name="cancel_appointment" class="btn  btn-sm cancel_appointment" data-id="'.$row["appointment_id"].'"><i class="fas fa-times"></i></button>';
